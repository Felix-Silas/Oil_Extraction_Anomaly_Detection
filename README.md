...
### 실제 구현 모델
위 상황을 해결하기 위해 특정 클래스에 필요한 특성만 고려하여 개별 이상 탐지기 구현한 후, 하나의 다중 클래스 이상 탐지기로 통합.  
(1) 3 vs 3이 아닌 경우를 분류해주는 이진 분류 모형 우선 개발  
(2) (1)의 분류기를 이용하여 3인지 3이 아닌지 먼저 분류  
(3) 3이 아닌 경우에 한해서 다른 이진 분류기 5개를 적용함  
(4) 모든 분류기에서 0이 아닌 경우에만 정상 데이터라고 분류  
